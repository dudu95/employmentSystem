...
namespace DAL
{
	...
	/// <summary>
	/// 实体类T_InvestmentStateService（可添加属性说明）
	/// </summary>
	public class T_InvestmentStateService
	{
		/// <summary>
		/// 计算当前表内的符合条件的所有数据的数量
		/// </summary>
		...
		public int DataCount(string safeSqlConditionInBLL = "")
		{
			try
			{
				string commandText = "select count(0) from T_InvestmentState WHERE " + safeSqlConditionInBLL;
		...
		/// </summary>
		/// <param name="T_InvestmentStateObject">从对象中提取中要查找的字段是否存在(对象方式是防止数据注入！)</param>
		/// <returns></returns>
		public Boolean IsExistWhereFeild(T_InvestmentState T_InvestmentStateObject)
		{
			try
			{
				string commandText = "select count(0) from T_InvestmentState WHERE stateName=@stateName";
				SqlParameter[] parms = new SqlParameter[]{
				new SqlParameter("@stateName",T_InvestmentStateObject.stateName),
				};
		...
		/// </summary>
		/// <param name="T_InvestmentStateObject">要插入的对象，屏蔽掉自增的字段</param>
		/// <returns></returns>
		public Boolean IsInsert(T_InvestmentState T_InvestmentStateObject)
		{
			try
			{
				string commandText = "INSERT INTO T_InvestmentState ( stateName) VALUES (@stateName)";
				SqlParameter[] parms = new SqlParameter[]{
				new SqlParameter("@stateName",T_InvestmentStateObject.stateName),
				};
		...
		/// </summary>
		/// <param name="T_InvestmentStateObject">要修改的结果对象，屏蔽掉自增的列，条件可修改</param>
		/// <returns></returns>
		public Boolean IsUpdate(T_InvestmentState T_InvestmentStateObject)
		{
			try
			{
				string commandText = "UPDATE T_InvestmentState SET stateName=@stateName WHERE investmentStateId=@investmentStateId";
				SqlParameter[] parms = new SqlParameter[]{
					new SqlParameter("@investmentStateId",T_InvestmentStateObject.investmentStateId),
					new SqlParameter("@stateName",T_InvestmentStateObject.stateName),
				};
				return SqlHelper_DG.ExecuteNonQuery(SqlHelper_DG.ConnString, commandText, CommandType.Text, parms) > 0 ? true : false;
			}
		...
		/// </summary>
		/// <param name="T_InvestmentStateObject">条件对象，唯一字段或者自定义删除条件</param>
		/// <returns></returns>
		public Boolean IsDelete(T_InvestmentState T_InvestmentStateObject)
		{
			try
			{
				string commandText = "DELETE FROM T_InvestmentState WHERE investmentStateId=@investmentStateId";
				SqlParameter[] parms = new SqlParameter[]{
				new SqlParameter("@investmentStateId",T_InvestmentStateObject.investmentStateId),
				};
				return SqlHelper_DG.ExecuteNonQuery(SqlHelper_DG.ConnString, commandText, CommandType.Text, parms) > 0 ? true : false;
		...
		/// </summary>
		/// <param name="T_InvestmentStateObject">条件对象，按需求来确定查找的条件</param>
		/// <returns></returns>
		public T SelectSingleLine_RTModel<T>(T_InvestmentState T_InvestmentStateObject)
		{
			try
			{
				string commandText = "SELECT TOP (1) * FROM T_InvestmentState WHERE investmentStateId=@investmentStateId";//这里需要按需求来确定需要查找的是哪个参数 因为要返回一行数据，所以搜索的条件值必须是唯一的，主键是最佳选择！
				SqlParameter[] parms = new SqlParameter[]{
				new SqlParameter("@investmentStateId",T_InvestmentStateObject.investmentStateId),
				};
        ...
        public T SelectByName_RTModel<T>(T_InvestmentState T_InvestmentStateObject)
		{
			try
			{
                string commandText = "SELECT TOP (1) * FROM T_InvestmentState WHERE stateName=@stateName";//这里需要按需求来确定需要查找的是哪个参数 因为要返回一行数据，所以搜索的条件值必须是唯一的，主键是最佳选择！
				SqlParameter[] parms = new SqlParameter[]{
				new SqlParameter("@stateName",T_InvestmentStateObject.stateName),
				};
				return SqlHelper_DG.ReturnModelByModels<T>(SqlHelper_DG.ExecuteReader(SqlHelper_DG.ConnString, commandText, CommandType.Text, parms));
		...
		public List<T> SelectALL<T>(string safeSqlCondition = " 1=1 ")
		{
			try
			{
				string commandText = "SELECT * FROM T_InvestmentState WHERE " + safeSqlCondition;//这里修改条件语句 默认全部
		...
		public List<T> SelectALLPaginByRowNumber<T>(int PageSize, int PageNumber, string DataOrderBy,string safeSqlCondition="1=1")
		{
			try
			{
				...
				commandText.Append("SELECT TOP " + PageSize + " * FROM (SELECT ROW_NUMBER() OVER (ORDER BY " + DataOrderBy + ") AS RowNumber,* FROM T_InvestmentState ");
