...
namespace DAL
{
	...
	/// Codes Builder Time (代码生成时间):2017-04-13 21:51:41
	/// </summary>

	/// <summary>
	/// 实体类T_EducationService（可添加属性说明）
	/// </summary>
	public class T_EducationService
	{
		/// <summary>
		/// 计算当前表内的符合条件的所有数据的数量
		/// </summary>
		...
		public int DataCount(string safeSqlConditionInBLL = "")
		{
			try
			{
				string commandText = "select count(0) from T_Education WHERE " + safeSqlConditionInBLL;
		...
		/// </summary>
		/// <param name="T_EducationObject">从对象中提取中要查找的字段是否存在(对象方式是防止数据注入！)</param>
		/// <returns></returns>
		public Boolean IsExistWhereFeild(T_Education T_EducationObject)
		{
			try
			{
				string commandText = "select count(0) from T_Education WHERE educationName=@educationName";
				SqlParameter[] parms = new SqlParameter[]{
				new SqlParameter("@educationName",T_EducationObject.educationName),
				};
		...
		/// </summary>
		/// <param name="T_EducationObject">要插入的对象，屏蔽掉自增的字段</param>
		/// <returns></returns>
		public Boolean IsInsert(T_Education T_EducationObject)
		{
			try
			{
				string commandText = "INSERT INTO T_Education ( educationName) VALUES (@educationName)";
				SqlParameter[] parms = new SqlParameter[]{
				new SqlParameter("@educationName",T_EducationObject.educationName),
				};
		...
		/// </summary>
		/// <param name="T_EducationObject">要修改的结果对象，屏蔽掉自增的列，条件可修改</param>
		/// <returns></returns>
		public Boolean IsUpdate(T_Education T_EducationObject)
		{
			try
			{
				string commandText = "UPDATE T_Education SET educationName=@educationName WHERE educationId=@educationId";
				SqlParameter[] parms = new SqlParameter[]{
					new SqlParameter("@educationId",T_EducationObject.educationId),
					new SqlParameter("@educationName",T_EducationObject.educationName),
				};
				return SqlHelper_DG.ExecuteNonQuery(SqlHelper_DG.ConnString, commandText, CommandType.Text, parms) > 0 ? true : false;
			}
		...
		/// </summary>
		/// <param name="T_EducationObject">条件对象，唯一字段或者自定义删除条件</param>
		/// <returns></returns>
		public Boolean IsDelete(T_Education T_EducationObject)
		{
			try
			{
				string commandText = "DELETE FROM T_Education WHERE educationId=@educationId";
				SqlParameter[] parms = new SqlParameter[]{
				new SqlParameter("@educationId",T_EducationObject.educationId),
				};
				return SqlHelper_DG.ExecuteNonQuery(SqlHelper_DG.ConnString, commandText, CommandType.Text, parms) > 0 ? true : false;
		...
		/// </summary>
		/// <param name="T_EducationObject">条件对象，按需求来确定查找的条件</param>
		/// <returns></returns>
		public T SelectSingleLine_RTModel<T>(T_Education T_EducationObject)
		{
			try
			{
				string commandText = "SELECT TOP (1) * FROM T_Education WHERE educationId=@educationId";//这里需要按需求来确定需要查找的是哪个参数 因为要返回一行数据，所以搜索的条件值必须是唯一的，主键是最佳选择！
				SqlParameter[] parms = new SqlParameter[]{
				new SqlParameter("@educationId",T_EducationObject.educationId),
				};
        ...
        /// <param name="T_EducationObject">条件对象，按需求来确定查找的条件</param>
        /// <returns></returns>
        public T SelectByName_RTModel<T>(T_Education T_EducationObject)
        {
            try
            {
                string commandText = "SELECT TOP (1) * FROM T_Education WHERE educationName=@educationName";
                SqlParameter[] parms = new SqlParameter[]{
				new SqlParameter("@educationName",T_EducationObject.educationName),
				};
                return SqlHelper_DG.ReturnModelByModels<T>(SqlHelper_DG.ExecuteReader(SqlHelper_DG.ConnString, commandText, CommandType.Text, parms));
		...
		public List<T> SelectALL<T>(string safeSqlCondition = " 1=1 ")
		{
			try
			{
				string commandText = "SELECT * FROM T_Education WHERE " + safeSqlCondition;//这里修改条件语句 默认全部
		...
		public List<T> SelectALLPaginByRowNumber<T>(int PageSize, int PageNumber, string DataOrderBy,string safeSqlCondition="1=1")
		{
			try
			{
				...
				commandText.Append("SELECT TOP " + PageSize + " * FROM (SELECT ROW_NUMBER() OVER (ORDER BY " + DataOrderBy + ") AS RowNumber,* FROM T_Education ");
