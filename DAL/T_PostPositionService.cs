...
namespace DAL
{
	...
	/// <summary>
	/// 实体类T_PostPositionService（可添加属性说明）
	/// </summary>
	public class T_PostPositionService
	{
		/// <summary>
		/// 计算当前表内的符合条件的所有数据的数量
		/// </summary>
		...
		public int DataCount(string safeSqlConditionInBLL = "")
		{
			try
			{
				string commandText = "select count(0) from T_PostPosition WHERE " + safeSqlConditionInBLL;
		...
		/// </summary>
		/// <param name="T_PostPositionObject">从对象中提取中要查找的字段是否存在(对象方式是防止数据注入！)</param>
		/// <returns></returns>
		public Boolean IsExistWhereFeild(T_PostPosition T_PostPositionObject)
		{
			try
			{
				string commandText = "select count(0) from T_PostPosition WHERE employerId=@employerId";
				SqlParameter[] parms = new SqlParameter[]{
				new SqlParameter("@employerId",T_PostPositionObject.employerId),
				};
		...
		/// </summary>
		/// <param name="T_PostPositionObject">要插入的对象，屏蔽掉自增的字段</param>
		/// <returns></returns>
		public Boolean IsInsert(T_PostPosition T_PostPositionObject)
		{
			try
			{
				string commandText = "INSERT INTO T_PostPosition ( employerId,position,workingLifeId,educationId,salary,advantage,describe) VALUES (@employerId,@position,@workingLifeId,@educationId,@salary,@advantage,@describe)";
				SqlParameter[] parms = new SqlParameter[]{
				new SqlParameter("@employerId",T_PostPositionObject.employerId),
				new SqlParameter("@position",T_PostPositionObject.position),
				new SqlParameter("@workingLifeId",T_PostPositionObject.workingLifeId),
				new SqlParameter("@educationId",T_PostPositionObject.educationId),
				new SqlParameter("@salary",T_PostPositionObject.salary),
				new SqlParameter("@advantage",T_PostPositionObject.advantage),
				new SqlParameter("@describe",T_PostPositionObject.describe),
				};
		...
		/// </summary>
		/// <param name="T_PostPositionObject">要修改的结果对象，屏蔽掉自增的列，条件可修改</param>
		/// <returns></returns>
		public Boolean IsUpdate(T_PostPosition T_PostPositionObject)
		{
			try
			{
				string commandText = "UPDATE T_PostPosition SET employerId=@employerId,position=@position,workingLifeId=@workingLifeId,educationId=@educationId,salary=@salary,advantage=@advantage,describe=@describe WHERE positionId=@positionId";
				SqlParameter[] parms = new SqlParameter[]{
					new SqlParameter("@positionId",T_PostPositionObject.positionId),
					new SqlParameter("@employerId",T_PostPositionObject.employerId),
					new SqlParameter("@position",T_PostPositionObject.position),
					new SqlParameter("@workingLifeId",T_PostPositionObject.workingLifeId),
					new SqlParameter("@educationId",T_PostPositionObject.educationId),
		...
		/// </summary>
		/// <param name="T_PostPositionObject">条件对象，唯一字段或者自定义删除条件</param>
		/// <returns></returns>
		public Boolean IsDelete(T_PostPosition T_PostPositionObject)
		{
			try
			{
				string commandText = "DELETE FROM T_PostPosition WHERE positionId=@positionId";
				SqlParameter[] parms = new SqlParameter[]{
				new SqlParameter("@positionId",T_PostPositionObject.positionId),
				};
				return SqlHelper_DG.ExecuteNonQuery(SqlHelper_DG.ConnString, commandText, CommandType.Text, parms) > 0 ? true : false;
		...
		/// </summary>
		/// <param name="T_PostPositionObject">条件对象，按需求来确定查找的条件</param>
		/// <returns></returns>
		public T SelectSingleLine_RTModel<T>(T_PostPosition T_PostPositionObject)
		{
			try
			{
				string commandText = "SELECT TOP (1) * FROM T_PostPosition WHERE positionId=@positionId";//这里需要按需求来确定需要查找的是哪个参数 因为要返回一行数据，所以搜索的条件值必须是唯一的，主键是最佳选择！
				SqlParameter[] parms = new SqlParameter[]{
				new SqlParameter("@positionId",T_PostPositionObject.positionId),
				};
		...
		public List<T> SelectALL<T>(string safeSqlCondition = " 1=1 ")
		{
			try
			{
				string commandText = "SELECT * FROM T_PostPosition WHERE " + safeSqlCondition;//这里修改条件语句 默认全部
        ...
        public List<T> SelectByEmployerId<T>(T_PostPosition T_PostPositionObject)
        {
            try
            {
                string commandText = "SELECT * FROM T_PostPosition WHERE employerId=@employerId";
                SqlParameter[] parms = new SqlParameter[]{
				    new SqlParameter("@employerId",T_PostPositionObject.employerId),
				};
		...
		public List<T> SelectALLPaginByRowNumber<T>(int PageSize, int PageNumber, string DataOrderBy,string safeSqlCondition="1=1")
		{
			try
			{
				...
				commandText.Append("SELECT TOP " + PageSize + " * FROM (SELECT ROW_NUMBER() OVER (ORDER BY " + DataOrderBy + ") AS RowNumber,* FROM T_PostPosition ");
