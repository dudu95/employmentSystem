...
namespace DAL
{
	...
	/// <summary>
	/// 实体类tb_AccountService（可添加属性说明）
	/// </summary>
	public class tb_AccountService
	{
		/// <summary>
		/// 计算当前表内的符合条件的所有数据的数量
		/// </summary>
		...
		public int DataCount(string safeSqlConditionInBLL = "")
		{
			try
			{
				string commandText = "select count(0) from tb_Account WHERE " + safeSqlConditionInBLL;
		...
		/// </summary>
		/// <param name="tb_AccountObject">从对象中提取中要查找的字段是否存在(对象方式是防止数据注入！)</param>
		/// <returns></returns>
		public Boolean IsExistWhereFeild(tb_Account tb_AccountObject)
		{
			try
			{
				string commandText = "select count(0) from tb_Account WHERE loginId=@loginId";
				SqlParameter[] parms = new SqlParameter[]{
				new SqlParameter("@loginId",tb_AccountObject.loginId),
				};
		...
		/// </summary>
		/// <param name="tb_AccountObject">要插入的对象，屏蔽掉自增的字段</param>
		/// <returns></returns>
		public Boolean IsInsert(tb_Account tb_AccountObject)
		{
			try
			{
				string commandText = "INSERT INTO tb_Account (loginId,pwd,email,sex,phone) VALUES (@loginId,@pwd,@email,@sex,@phone)";
				SqlParameter[] parms = new SqlParameter[]{
				new SqlParameter("@loginId",tb_AccountObject.loginId),
				new SqlParameter("@pwd",tb_AccountObject.pwd),
				new SqlParameter("@email",tb_AccountObject.email),
				new SqlParameter("@sex",tb_AccountObject.sex),
				new SqlParameter("@phone",tb_AccountObject.phone),
				};
                int issuccess = SqlHelper_DG.ExecuteNonQuery(SqlHelper_DG.ConnString, commandText, CommandType.Text, parms);
		...
		/// </summary>
		/// <param name="tb_AccountObject">要修改的结果对象，屏蔽掉自增的列，条件可修改</param>
		/// <returns></returns>
		public Boolean IsUpdate(tb_Account tb_AccountObject)
		{
			try
			{
				string commandText = "UPDATE tb_Account SET ID=@ID,loginId=@loginId,pwd=@pwd,email=@email,sex=@sex,phone=@phone WHERE ID=@ID";
				SqlParameter[] parms = new SqlParameter[]{
					new SqlParameter("@ID",tb_AccountObject.ID),
					new SqlParameter("@ID",tb_AccountObject.ID),
					new SqlParameter("@loginId",tb_AccountObject.loginId),
					new SqlParameter("@pwd",tb_AccountObject.pwd),
					new SqlParameter("@email",tb_AccountObject.email),
		...
		/// </summary>
		/// <param name="tb_AccountObject">条件对象，唯一字段或者自定义删除条件</param>
		/// <returns></returns>
		public Boolean IsDelete(tb_Account tb_AccountObject)
		{
			try
			{
				string commandText = "DELETE FROM tb_Account WHERE ID=@ID";
				SqlParameter[] parms = new SqlParameter[]{
				new SqlParameter("@ID",tb_AccountObject.ID),
				};
				return SqlHelper_DG.ExecuteNonQuery(SqlHelper_DG.ConnString, commandText, CommandType.Text, parms) > 0 ? true : false;
		...
		/// </summary>
		/// <param name="tb_AccountObject">条件对象，按需求来确定查找的条件</param>
		/// <returns></returns>
		public T SelectSingleLine_RTModel<T>(tb_Account tb_AccountObject)
		{
			try
			{
				string commandText = "SELECT TOP (1) * FROM tb_Account WHERE ID=@ID";//这里需要按需求来确定需要查找的是哪个参数 因为要返回一行数据，所以搜索的条件值必须是唯一的，主键是最佳选择！
				SqlParameter[] parms = new SqlParameter[]{
				new SqlParameter("@ID",tb_AccountObject.ID),
				};
		...
		public List<T> SelectALL<T>(string safeSqlCondition = " 1=1 ")
		{
			try
			{
				string commandText = "SELECT * FROM tb_Account WHERE " + safeSqlCondition;//这里修改条件语句 默认全部
		...
		public List<T> SelectALLPaginByRowNumber<T>(int PageSize, int PageNumber, string DataOrderBy,string safeSqlCondition="1=1")
		{
			try
			{
				...
				commandText.Append("SELECT TOP " + PageSize + " * FROM (SELECT ROW_NUMBER() OVER (ORDER BY " + DataOrderBy + ") AS RowNumber,* FROM tb_Account ");
