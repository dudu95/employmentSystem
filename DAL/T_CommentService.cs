...
namespace DAL
{
	...
	public class T_CommentService
	{
		...
		public int DataCount(string safeSqlConditionInBLL = "")
		{
			try
			{
				string commandText = "select count(0) from T_Comment WHERE " + safeSqlConditionInBLL;
		...
		/// </summary>
		/// <param name="T_CommentObject">从对象中提取中要查找的字段是否存在(对象方式是防止数据注入！)</param>
		/// <returns></returns>
		public Boolean IsExistWhereFeild(T_Comment T_CommentObject)
		{
			try
			{
				string commandText = "select count(0) from T_Comment WHERE employeeId=@employeeId";
				SqlParameter[] parms = new SqlParameter[]{
				new SqlParameter("@employeeId",T_CommentObject.employeeId),
				};
		...
		/// </summary>
		/// <param name="T_CommentObject">要插入的对象，屏蔽掉自增的字段</param>
		/// <returns></returns>
		public Boolean IsInsert(T_Comment T_CommentObject)
		{
			try
			{
				string commandText = "INSERT INTO T_Comment ( employeeId,employerId,comment) VALUES (@employeeId,@employerId,@comment)";
				SqlParameter[] parms = new SqlParameter[]{
				new SqlParameter("@employeeId",T_CommentObject.employeeId),
				new SqlParameter("@employerId",T_CommentObject.employerId),
				new SqlParameter("@comment",T_CommentObject.comment),
				};
		...
		/// </summary>
		/// <param name="T_CommentObject">要修改的结果对象，屏蔽掉自增的列，条件可修改</param>
		/// <returns></returns>
		public Boolean IsUpdate(T_Comment T_CommentObject)
		{
			try
			{
				string commandText = "UPDATE T_Comment SET employeeId=@employeeId,employerId=@employerId,comment=@comment WHERE commentId=@commentId";
				SqlParameter[] parms = new SqlParameter[]{
					new SqlParameter("@commentId",T_CommentObject.commentId),
					new SqlParameter("@employeeId",T_CommentObject.employeeId),
					new SqlParameter("@employerId",T_CommentObject.employerId),
					new SqlParameter("@comment",T_CommentObject.comment),
				};
		...
		/// </summary>
		/// <param name="T_CommentObject">条件对象，唯一字段或者自定义删除条件</param>
		/// <returns></returns>
		public Boolean IsDelete(T_Comment T_CommentObject)
		{
			try
			{
				string commandText = "DELETE FROM T_Comment WHERE commentId=@commentId";
				SqlParameter[] parms = new SqlParameter[]{
				new SqlParameter("@commentId",T_CommentObject.commentId),
				};
				return SqlHelper_DG.ExecuteNonQuery(SqlHelper_DG.ConnString, commandText, CommandType.Text, parms) > 0 ? true : false;
		...
		/// </summary>
		/// <param name="T_CommentObject">条件对象，按需求来确定查找的条件</param>
		/// <returns></returns>
		public T SelectSingleLine_RTModel<T>(T_Comment T_CommentObject)
		{
			try
			{
				string commandText = "SELECT TOP (1) * FROM T_Comment WHERE commentId=@commentId";//这里需要按需求来确定需要查找的是哪个参数 因为要返回一行数据，所以搜索的条件值必须是唯一的，主键是最佳选择！
				SqlParameter[] parms = new SqlParameter[]{
				new SqlParameter("@commentId",T_CommentObject.commentId),
				};
		...
		public List<T> SelectALL<T>(string safeSqlCondition = " 1=1 ")
		{
			try
			{
				string commandText = "SELECT * FROM T_Comment WHERE " + safeSqlCondition;//这里修改条件语句 默认全部
        ...
        public List<T> SelectByEmployeeId<T>(T_Comment T_CommentObject)
        {
            try
            {
                string commandText = "SELECT * FROM T_Comment WHERE employeeId=@employeeId";
                SqlParameter[] parms = new SqlParameter[]{
				    new SqlParameter("@employeeId",T_CommentObject.employeeId),
				};
        ...
        public List<T> SelectByEmployerId<T>(T_Comment T_CommentObject)
        {
            try
            {
                string commandText = "SELECT * FROM T_Comment WHERE employerId=@employerId";
                SqlParameter[] parms = new SqlParameter[]{
				    new SqlParameter("@employerId",T_CommentObject.employerId),
				};
                return SqlHelper_DG.ReturnListByModels<T>(SqlHelper_DG.ExecuteDataSet(SqlHelper_DG.ConnString, commandText, CommandType.Text, parms));
		...
		public List<T> SelectALLPaginByRowNumber<T>(int PageSize, int PageNumber, string DataOrderBy,string safeSqlCondition="1=1")
		{
			try
			{
				...
				commandText.Append("SELECT TOP " + PageSize + " * FROM (SELECT ROW_NUMBER() OVER (ORDER BY " + DataOrderBy + ") AS RowNumber,* FROM T_Comment ");
