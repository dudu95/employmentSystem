...
namespace BLL
{
	...
	/// <summary>
	/// 实体类T_EmployeeManager（可添加属性说明）
	/// </summary>
	public class T_EmployeeManager
	{
		//返回表中的数据数量 Int 一般配合分页使用
		public int DataCount()
		{
			string where = " 1=1 ";//判断条件语句可以自由发挥,默认返回全部 必须写安全的sql语句，防止数据注入!!!
			return new T_EmployeeService().DataCount(where);
		}
		//返回是否存在
		public Boolean IsExistWhereFeild(T_Employee T_EmployeeObject)
		{
			return new T_EmployeeService().IsExistWhereFeild(T_EmployeeObject);//这个需要按项目需求修改DAL层的条件代码以符合项目！！！
		}
		//插入业务
		public Boolean IsInsert(T_Employee T_EmployeeObject)
		{
			return new T_EmployeeService().IsInsert(T_EmployeeObject);//自动过滤掉自增字段
		}
		//修改业务
		public Boolean IsUpdate(T_Employee T_EmployeeObject)
		{
			return new T_EmployeeService().IsUpdate(T_EmployeeObject);//条件写在DAL层代码中
		}
		//删除业务
		public Boolean IsDelete(T_Employee T_EmployeeObject)
		{
			return new T_EmployeeService().IsDelete(T_EmployeeObject);//条件写在DAL层代码中
		}
		//获取到某一行的业务--返回是Model类型的数据
		public T_Employee SelectSingleLine_RTModel(T_Employee T_EmployeeObject)
		{
		...
		{
			return new T_EmployeeService().SelectALL<T_Employee>();//这里补充返回Model的条件，为空默认所有数据
		}
		//分页获取到符合条件的所有值的业务，一般配合返回总数的方法使用显示总页数！--返回List T
		public List<T_Employee> SelectALLPaginByRowNumber(int PageSize, int PageNumber, string DataOrderBy)
		{
			string where = " 1=1 ";//判断条件语句可以自由发挥,默认返回全部 必须写安全的sql语句，防止数据注入!!!
			return new T_EmployeeService().SelectALLPaginByRowNumber<T_Employee>(PageSize,PageNumber,DataOrderBy,where);
		}
	}
}
